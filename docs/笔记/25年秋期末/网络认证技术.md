...
2. CRL的基本内容：版本，issuer名字，时间（thisUpdate, nextUpdate），证书的撤销时间和序列号；CRL扩展
3. CRL的验证方式：拆分排序的CRL文件
4. 重定向RCRL：证书的CRL可能不固定，该CRL只记录真实的CRL在哪里找
5. OCSP的缺点：需要多一次数字签名验证；直接和CA通信可能导致安全问题，和CRL通信不能解决时效性问题。
6. CRT：使用树来存储，列表长度短所以下载量为 logn，查询也为logn；不需要多一层OCSP的数字签名，响应速度和CRL类似
7. 短周期证书回避了CRL，但是增加了CA的负担；但它需要用户每天都去更新证书。
8. 证书撤销存在延迟；默认假设是当查询撤销状态不可得时默认认为证书仍未被撤销。
